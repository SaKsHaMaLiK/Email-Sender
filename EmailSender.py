...
def Login():
    ...
    if '@gmail.com' not in e or e == "":
        ...
        try:
            ...
            s.login(e, p)
            messagebox.showinfo(
                "Login Success", "You have Logged to Gmail Successfully")
            ...
            h2 = Label(root, text="Email Sender", bg="orange",
                       fg="black", font=('verdana', 13, 'bold'))
            h2.place(x=175, y=5)

            logout = Button(root, text="Logout", padx=20, bg="orange", relief=RIDGE, borderwidth=1, font=(
                'verdana', 10, 'bold'), cursor="hand2", command=Logout)
            logout.place(x=390, y=38)

            r = Label(root, text="Recipetent Email Address",
            ...
            message = ScrolledText(
                root, width=40, height=5, relief=RIDGE, borderwidth=3)
            ...
            def Send():
                ...
                if '@gmail.com' not in r or r == "":
                    messagebox.showerror(
                        'Sending Mail error', "Please Write the Valid Email")
                elif m == "":
                    messagebox.showerror(
                        'Sending Mail error', "Message shouldn't be Empty")
            ...
            send = Button(root, text="Send", padx=30, relief=RIDGE, borderwidth=1, bg="orange", font=(
                'verdana', 10, 'bold'), cursor="hand2", command=Send)
            send.place(x=350, y=360)
            root.mainloop()

        except:
            messagebox.showerror(
                'Login error', "Failed to Login, Either Your Email or Password is Wrong nor You did Enable less secure Apps in gmail Setting")
...
h1 = Label(root, text="Email Sender", bg="orange",
           fg="black", font=('verdana', 13, 'bold'))
h1.place(x=135, y=5)
...
login = Button(root, text="Login", padx=30, bg="orange", relief=RIDGE,
               borderwidth=1, font=('verdana', 10, 'bold'), cursor="hand2", command=Login)
